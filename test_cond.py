...
if max_condition == 0:
    raise Exception("E: num_of_speaker and num_of_noise can't 0 same time in test_cond.py")
...
for i in range(len(test_source_file_list)):
    ...
    noise_condition_num = int(test_source_file_list[i].replace(".wav", "").split('_')[-1])
    ...
    speaker_condition_num = int(test_source_file_list[i].replace(".wav", "").split('_')[-2])
    ...
    for j in range(number_of_frame):
        if window_type != "uniform":
            ...
            test_source_condition_list.append(tf.concat([tf.one_hot(noise_condition_num, num_of_noise), tf.one_hot(speaker_condition_num, num_of_speaker)],0).numpy().tolist())
        else:
