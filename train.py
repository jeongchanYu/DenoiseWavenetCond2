...
with strategy.scope():
    ...
    train_source_cut_list, train_source_condition_list, train_target_cut_list, number_of_total_frame_train = md.make_dataset(train_source_path, train_target_path,
                                                                 previous_size, frame_size, future_size, 0,
                                                                 0, shift_size, window_type)
    train_dataset = tf.data.Dataset.from_tensor_slices((train_source_cut_list, train_target_cut_list)).shuffle(number_of_total_frame_train).batch(batch_size)
    ...
    if early_stopping:
        test_source_cut_list, test_source_condition_list, test_target_cut_list, number_of_total_frame_test = md.make_dataset(test_source_path, test_target_path,
                                                                   ...
                                                                   0, 0, shift_size,
                                                                   window_type)
        test_dataset = tf.data.Dataset.from_tensor_slices((test_source_cut_list, test_target_cut_list)).shuffle(number_of_total_frame_test).batch(batch_size)
